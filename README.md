<p align="center">
  <img src="assets/img/readme/banner.webp" alt="salmantahir.net, UI/UX designer and front-end developer" width="100%">
</p>

<p align="center">
  <a href="https://salmantahir.net"><strong>View the site&nbsp;&nbsp;↗</strong></a>
</p>
...
| **Shape** | `corner-shape: squircle` with a `border-radius` fallback, one radius scale site-wide |
...
Every page is roughly 120 KB on first load and the whole deployable site is under 2 MB.
...
assets/
  ...
  img/                     site-level imagery
  img/<slug>/              per project: feature image, screenshots, logos
...
1. **Create `work/<slug>/index.html`.** Copy an existing case study as the starting point. The page
   shell, meaning head, sprite, header, footer and script tags, is identical on every page and
   should stay that way.
2. **Put imagery in `assets/img/<slug>/`,** matching the URL slug. A `feature.webp` at 16:10 for the
   grid and the page hero, plus whatever screenshots the write-up needs. WebP, sized to what it is
   actually painted at, `width` and `height` always set.
3. **Add the card** to `work/index.html` and, if it is a headline project, to the home page.
   `data-tags` on the card is what the filter reads.
4. **Register the URL** in `sitemap.xml`.

Disciplines are a closed set, shared by the content tags and the filter chips:

`brand` · `research` · `ui` · `web` · `dev` · `testing` · `ecom` · `seo`

Each has an SVG symbol in the sprite at the top of every page. Adding a discipline means adding a
symbol and a filter chip, not inventing a one-off label.

Feature images sit on a paper-grain ground tinted from the project's own brand colour, with the
screenshot placed differently per project so a grid of them does not read as six of the same
picture.
...
Cloudflare Pages, connected to this repository. Pushing to `main` deploys.
...
The contact form needs one environment variable, `RESEND_API_KEY`, set under
**Settings → Environment variables**, and the sending domain verified in Resend. Replies go to
whoever filled in the form rather than to the site.

## Licence

[MIT](LICENSE) for the code. The writing, photography and project imagery are mine and are not
covered by it.
